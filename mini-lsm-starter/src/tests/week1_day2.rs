...
fn test_task1_memtable_iter() {
...

    {
        ...
        let dd = iter.key().for_testing_key_ref();
        assert_eq!(dd, b"key1");
...
#[test]
fn test_task2_merge_3() {
    let mut iter1 = MockIterator::new(vec![
        (Bytes::from("1"), Bytes::from("233333")),
        (Bytes::from("3"), Bytes::from("233333")),
    ]);
    let mut iter2 = MockIterator::new(vec![
        (Bytes::from("1"), Bytes::from("del")),
        (Bytes::from("2"), Bytes::from("del")),
        (Bytes::from("3"), Bytes::from("2333")),
        (Bytes::from("4"), Bytes::from("23333")),
    ]);
    let mut iter3 = MockIterator::new(vec![
...
fn test_task4_integration() {
    ...
    assert_eq!(
        storage.get(b"3").unwrap().unwrap(),
        Bytes::copy_from_slice(b"233333")
    );
